import AddIcon from "@material-ui/icons/Add";
import React from 'react';

import ToolbarComponent from '../../common/Toolbar';
import Table from "../../common/Table";

const ListClient = props => (
    <div className="list row">
        <Table 
            rows={props.clients}
            headCells={[
            {id: 'name', numeric: false, disablePadding: true, label: 'Nom'},
            {id: 'siren', numeric: true, disablePadding: false, label: 'Numéro SIREN'},
            {id: 'siret', numeric: true, disablePadding: false, label: 'Numéro SIRET'},
            ]}
            sortKey='id'
            tableTitle='Liste des clients'
            handleOnClickDelete={props.deleteClients}
            handleOnClickEdit={props.editClient}
        />
            
            <ToolbarComponent fabIcon={<AddIcon/>} handleFabClick={props.navigateAddClient} color="secondary"/>
    </div>
);

export default ListClient;